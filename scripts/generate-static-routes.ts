...
const toolsMetadata: Record<string, { title: string; description: string; keywords: string }> = {
  '/': {
    title: 'FreeDevTool.App - Free, Secure, Open Source, and Offline Developer Tools',
    description: 'Open source offline developer tools designed for privacy. Free utilities including converters, formatters, encoders, text tools, and hardware testing - built to work without network requests.',
    keywords: 'offline developer tools, privacy-focused dev utilities, open source coding tools, no network requests, local processing developer tools, privacy-first utilities'
  },
...
function extractAssetReferences(distPath: string): { jsFile: string; cssFile: string } {
  const rootIndexPath = path.join(distPath, 'index.html');
  const rootHtml = fs.readFileSync(rootIndexPath, 'utf-8');
  
  // Extract JS file
  const jsMatch = rootHtml.match(/<script[^>]+src="([^"]+index[^"]+\.js)"[^>]*>/);
  const jsFile = jsMatch ? jsMatch[1] : '/assets/index.js';
  
  // Extract CSS file  
  const cssMatch = rootHtml.match(/<link[^>]+href="([^"]+index[^"]+\.css)"[^>]*>/);
...
function generateRouteHTML(route: string, distPath: string, assets: { jsFile: string; cssFile: string }) {
