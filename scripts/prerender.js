...
const toolPaths = JSON.parse(fs.readFileSync(path.resolve(PROJECT_ROOT, 'client/src/data/tools-paths.json'), 'utf-8'));
...
async function generateStaticPages() {
  ...
  for (const toolPath of allPaths) {
    ...
    html = html.replace(/="\/([^"]*\.(js|css|png|ico|xml|txt))"/g, `="${prefix}$1"`);
    
    // Update page title to include tool name
    const toolName = toolPath.split('/').pop().replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
    html = html.replace(
      /<title>([^<]*)<\/title>/,
